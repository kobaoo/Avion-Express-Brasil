-- 1. Общее количество заказов и выручка по месяцам
...
-- 2. Среднее время доставки заказа (от покупки до фактической доставки)
...
-- 5. Средняя оценка покупателей по категориям товаров
SELECT p.product_category_name,
       AVG(orv.review_score) AS avg_score
...
-- 6. Доля способов оплаты в общем числе платежей
SELECT payment_type,
...
-- 7. Средний чек (сумма заказа) по штатам (customer_state)
SELECT c.customer_state,
       AVG(t.order_total) AS avg_order_total
FROM (
...
-- 10. Самые популярные категории товаров по количеству заказанных позиций
SELECT p.product_category_name,
